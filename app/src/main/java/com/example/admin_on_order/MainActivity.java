...
//            builder.addText(printOrderModel.getNotice());
    ...
    public void changeCheckStatus(int position, Context context, String key) {
        ...
        String time2 = format2.format(calendar.getTime());
        FirebaseDatabase.getInstance().getReference().child("order").child("신화돌담집").child(time2).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                for (DataSnapshot item : snapshot.getChildren()) {
                    if (item.getKey().equals(key)) {
                        ...
                        FirebaseDatabase.getInstance().getReference().child("order").child("신화돌담집").child(time2).child(item.getKey()).setValue(printOrderModel);
                    }
